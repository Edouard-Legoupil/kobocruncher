...
When analyzing those representations in a collective setting during data interpretation sessions, the group shall:  
 ...
 A typical data interpretation session shall not last more than 2hours and include more than 60 visuals to look at in order to keep participants with a good focus level.
...
## You do not need to be expert as soon as you follow a recipes. 

KoboCruncher is an R package designed to take care of the processing component so that the technical team can focus on the interpretation.


 * __Productivity__: Once the configuration file is written, run the report in Rstudio to get the output
 
 * __Training__: No need to write R instruction. Indeed only a limited knowledge of R is required
 
 * __Iteration__: Check the output, adjust the various input files & re-run the report till you get a satisfying report
...
##  The sessions are organised to guide you through the different steps:
  ...
  *  [06-Cleaning_and_Indicator_calculation](06-Cleaning_and_Indicator_calculation.html) after reviewing the first iteration of the reports, we can identify potential data quality issue to fix and create calculated variable and indicator 
  ...
  *  [08-Weighting](08-Weighting.html) If the data was created through a probabilistic selection sampling approach, then we can apply weighting to the data before and regenerate the report so that those weights are reflected
  *  [09-Publishing](9-Publishing.html) The final stage of the process implies to facilitate both the interpretation of the data and the creation of data story telling content 
...
Do not hesitate to raise your questions in the ticket system so the training content can be improved accordingly
