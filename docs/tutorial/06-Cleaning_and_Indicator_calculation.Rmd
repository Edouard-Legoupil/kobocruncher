...
In order to guide this selection phase, data experts, in collaboration with the data analysis group, can use the following elements:
  ...
  * Use cross-tabulation to verify potential illogical combination of answers (for instance "pregnant men"... )
  ...
  *  Always, Check for missing data (NA) or "%of respondent who answered" (in the chart caption) that you cannot confidently explain
...
### Variable cleaning
  ...
  *  Removing direct identifiers such as: 
...
.large[Fix them] by setting the concerned variable as `identifier` in the `anonymise` column the `xlsform`:
