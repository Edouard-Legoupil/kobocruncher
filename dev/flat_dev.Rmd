...
### Survey sheet ######################################
    ...
    ## Clean the lable and hint in relation with the selected language 
    ...
    if ("clean" %in% colnames(survey)) {  } else {  survey$clean <- "no" }
    if ("anonymise" %in% colnames(survey)) {  } else {survey$anonymise <- "default-non-anonymised"}
...
### Choices sheet ######################################
    ...
    if ("labelReport" %in% colnames(choices)) { } else {choices["labelReport"] <- substr(choices[,"label"],1,50)}
    if ("score" %in% colnames(score)) {  } else { choices$score <- ""}
    namesOfCho <- c("list_name", "name", "label", "labelReport", "order", "score")
...
### Saving final ######################################    
    ...
    cat("\n******************** The XLSFORM has now been extended *********************\n \n")
...
kobo_prepare_form( xlsformpath = system.file("sample_xlsform.xlsx", package = "kobocruncher"),
                   xlsformpathout = "sample_xlsform_with_plan.xlsx" )
...
#' See a more detailed presentation of that step here: https://www.youtube.com/watch?v=0jE-Y7g88K4&feature=youtu.be&t=2305  
#'
#' @rdname template_2_interpretation
#'
...
#' 
#' @rdname template_3_dissemination
#'
