...
plot_select_one <- function(datapath  ,
  ...
  if (rr != 0 ) {
    ...
    p <- ggplot(cnts, aes(p, x)) +
      ...
      scale_y_discrete(labels = function(x, var) {label_choiceset(xlsformpath = xlsformpath, x= rlang::as_string(rlang::expr(var)) )(x) |>
                      stringr::str_wrap(40)}) +
...
plot_integer_cross <- function(datapath = datapath, 
  ...
  if (rr != 0 ) { 
    ...
    p <- ggplot(data) + 
      geom_boxplot(aes(x = .data[[var]], y =  .data[[crosstab]]), 
                     fill = "#0072BC", 
                     color = "white" 
      ) +
      # scale_y_continuous(labels = scales::label_percent()) +
...
plot_correlation <- function(datapath = datapath, 
 ...
 p <-   corrplot::corrplot(stats::chisq.test(MainDataFrame$monit.FamiliyLeft,MainDataFrame$monit.RouteIncident)$residuals,
